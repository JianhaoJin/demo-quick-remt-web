<template>
  <div class="content">
    ...
    <el-form ref="form" :model="form" label-width="170px" :rules="rules" v-loading="formLoading">
      ...
      <el-form-item v-if="form.term != 'T+0'" label="担保收取方式：" prop="way">
        <el-select
          v-model="form.way"
          placeholder="----请选择----"
      ...
      <el-form-item v-if="form.term != 'T+0'" label=" " label-width="50px">
        <div v-if="form.way">
  ...
  data() {
    ...
    return {
      ...
      form: {
        ...
        frxRate: "",
        way: "",
        priAcct: "31012025000017091",//主币种账号(根据实际需求进行更改)
        secAcct: "82260120102022631",//次币种账号(根据实际需求进行更改)
        priAmt: "",
        secAmt: "",
      ...
      rules: {
        ...
        aisle: [
          { required: true, message: "请选择是否绿色通道", trigger: "change" }
        ],
        bailAcctNo: [
  ...
  methods: {
    ...
    getData() {
      ...
      quotedPrice(params).then(res => {
        ...
        if (res.data.returnCode === "000000") {
          this.form.frxRate = res.data.rate;
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          let params = {
            ...
            channelType: "YQ", //渠道类型
            // cifName: "144524030", //客户名称
            custName: '1', //客户名称(根据实际需求进行更改)
            cifNo: "0000005332", //客户号(根据实际需求进行更改)
          ...
          // 如果担保方式选择 保证金 则需要添加参数 bail
          // 如果担保方式选择 金市通 则需要添加参数 goldMarket
          // 如果担保方式选择 授信 则需要添加参数 planCredit
          // 如果担保方式选择 保证金+授信 则需要添加参数 bail和goldMarket
          ...
          if(this.form.term === 'T+0')  params.way = ''
          switch (params.way) {
            case "保证金":
              params.bail = this.bailList.filter(v => {
              ...
              break;
            case "金市通":
              params.goldMarket = this.goldMarketList.filter((v,index) => {
                if(index === 0) {
          ...
          otherClickTrade(params).then(res => {
            this.formLoading = false;
            if (res.data.returnCode === "000000") {
