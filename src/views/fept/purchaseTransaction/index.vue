<template>
  <div class="content">
    ...
    <el-form ref="form" :model="form" label-width="170px" :rules="rules" v-loading="formLoading">
      ...
      <el-form-item v-if="form.term === 'T+0'" label="是否绿色通道：" prop="aisle">
        <el-select
          v-model="form.aisle"
          placeholder="----请选择----"
          @change="selectChange($event,'aisle')"
        >
    ...
    <!-- 绿色通道入口弹窗 -->
    <greenDialog v-if="greenVisible" @dialogClose="dialogClose" />
  </div>
</template>
...
import greenDialog from "./components/greenDialog.vue"; //引入绿色通道弹窗组件
export default {
  name: "demo",
  components: {
    greenDialog
    ...
    submit(flag) {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          // 如果选择绿色通道，则需要添加当前选中的合同编号参数 contractList
          if (params.aisle === "是" && this.form.term === 'T+0') {
            params.contractList = this.contractList;
          }
          ...
          // func: purchasing t0点击成交和 otherClickTrade 非t0点击成交的方法
          // funcIndex: 通过用户选择的期限来决定要使用哪一个方法进行提交
          const func = [purchasing,otherClickTrade],
